        ...
        , vm_(vm)
{
    ...
    active_element_ = std::unique_ptr<Button>(
        new Button(x, y, width, height, &font_, list[ default_index ], CalcFontSize(vm_, 140),
                   BTN_DDL_COLOR_TXT_IDLE, BTN_DDL_COLOR_TXT_HOVER, BTN_DDL_COLOR_TXT_ACTIVE,
                   BTN_DDL_COLOR_BG_IDLE, BTN_DDL_COLOR_BG_HOVER, BTN_DDL_COLOR_BG_ACTIVE));
    ...
    for (size_t i = 0; i < num_of_elements; i++)
    {
        list_.push_back(std::unique_ptr<Button> (
            new Button( x, (y + (i + 1) * height * 0.8f), width, height * 0.8f, &font_, list[ i ], CalcFontSize(vm_, 140),
                       BTN_DDL_COLOR_TXT_IDLE, BTN_DDL_COLOR_TXT_HOVER, BTN_DDL_COLOR_TXT_ACTIVE, BTN_DDL_COLOR_BG_IDLE,
                       BTN_DDL_COLOR_BG_HOVER, BTN_DDL_COLOR_BG_ACTIVE, i)));
    }
}
